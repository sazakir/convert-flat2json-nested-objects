...
namespace consoleExpandoObj
{
    class Program
    {
        const string REQUIREMENT_OBJ = "requirement";
        const string ACTIVTY_OBJ = "activity";
        const string TASK_OBJ = "task";
        ...
        {
            inputText = "m1,    c1,     req1,   act1,   not started, 11, task 1.1;" +
            ...
            objNames = new List<string>() { REQUIREMENT_OBJ, ACTIVTY_OBJ, TASK_OBJ };

            // stored as parent, child strings
            objRels = new Dictionary<string, string>();
            objRels.Add(string.Empty, REQUIREMENT_OBJ);
            objRels.Add(REQUIREMENT_OBJ, ACTIVTY_OBJ);
            objRels.Add(ACTIVTY_OBJ, TASK_OBJ);

            objDefs = new Dictionary<string, List<string>>();
            //Requirement
            var fldList = new List<string>() { "__rowid", "module", "component", "title" };
            objDefs.Add(REQUIREMENT_OBJ, fldList);
            //Activity
            fldList = new List<string>() { "__rowid", "act_name", "act_status" };
            objDefs.Add(ACTIVTY_OBJ, fldList);
            //Task
            fldList = new List<string>() { "__rowid", "t_id", "t_desc" };
            objDefs.Add(TASK_OBJ, fldList);
